...
/**
 * WorkerTask on the source side，this worker is responsible for writing data to the messaging
 * system.
 */
public class WorkerSourceTask extends WorkerTask {
  ...
  /**
   * This method will send a batch of message to the messaging system, and if it fails, it can be
   * resent.
   *
  ...
  private boolean sendMessages() {
    ...
    for (SourceDataEntry dataEntry : toSend) {
      ...
      for (Object o : dataEntry.getPayload()) {
        payloads.add(ConvertUtils.getBytesfromObject(o));
      }
      MessageBody messageBody = new MessageBody();
      messageBody.setTopic(dataEntry.getQueueName());
      messageBody.setPartition(
          ConvertUtils.getObjectFromBytes(sourcePartition.array(), String.class));
      messageBody.setPosition(ConvertUtils.getObjectFromBytes(sourcePosition.array(), Map.class));
      messageBody.setSchema(schema);
      messageBody.setPayload(dataEntry.getPayload());
      Message message =
          this.producer.createBytesMessage(
              dataEntry.getQueueName(), ConvertUtils.getBytesfromObject(messageBody));

      // If we fail to send, we will not re-add the failed message to the map, but send it directly
      // again.
      synchronized (this) {
        if (!lastFailed) {
          if (flushing) {
      ...
      try {
        ...
        processed++;
        lastFailed = false;
        thisBatch.add(messageBody.toString());
    ...
    log.info("Message is :{}", String.join(" \\n ", thisBatch));
    toSend.clear();
    return true;
  }
  ...
  public void commitPosition() {
    ...
    long commitTimeLimit =
        this.workerConfig.getWorkerConfig().getLong(WorkerConfig.POSITION_COMMIT_TIMEOUT_MS_CONFIG);
    synchronized (this) {
      this.flushing = true;
