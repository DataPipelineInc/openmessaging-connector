...
public class StandaloneProcessor extends AbstractProcessor {
  private static final Logger log = LoggerFactory.getLogger(StandaloneProcessor.class);
  ...
  public void putConnectorConfig(
    ...
    try {
      startConnector(connectorName, config);
    } catch (Exception e) {
      callBack.onCompletion(
          new ConnectException("Failed to start the connector : " + connectorName, e), null);
      return;
    }
  ...
  private void startConnector(String connectorName, Map<String, String> config) {
    try {
      ...
      configStorageService.removeConnectorConfig(connectorName);
  ...
  public void restartConnector(String connector, CallBack<Void> callBack) {
    ...
    worker.stopConnector(connector);
    try {
      worker.startConnector(connector, connectorConfig, targetState, this.connectorStatusListener);
